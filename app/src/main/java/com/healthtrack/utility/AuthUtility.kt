...
import com.healthtrack.utility.authsp.AuthSpFactory
import com.healthtrack.views.PermissionActivity
...
object AuthUtility {
    ...
    fun updateToken(): String = try {
        ...
        val client = com.healthtrack.network.NetworkClient.getRetrofitClient(
            true,
            true,
            true,
            AUTH_BASE_URL,
        ...
        val call = client.create(com.healthtrack.network.PostDataInterface::class.java)
...
//        })
    ...
    fun verifyOtp(mobile: String, otp: String, listener: UserVerifyListener?) {
        ...
        val validateOTP =
            com.healthtrack.models.network.ValidateOTP(mobile, otp)
        val map = mutableMapOf<String, String>()
        map[Constants.X_API_KEY] = BuildConfig.AWS_API_KEY
        map[Constants.OS] = Build.VERSION.SDK_INT.toString()
        map[Constants.DEVICE_TYPE] = Build.MANUFACTURER + Constants.HYPHEN + Build.MODEL
...
//        call.enqueue(object :
//            retrofit2.Callback<com.healthtrack.models.network.TokenValidationResponse> {
//            override fun onFailure(
//                call: Call<com.healthtrack.models.network.TokenValidationResponse>,
//                t: Throwable
//            ) {
//                val e = Exception(t)
...
//                call: Call<com.healthtrack.models.network.TokenValidationResponse>,
//                response: Response<com.healthtrack.models.network.TokenValidationResponse>
//            ) {
    ...
    fun logout(context: Context, moveToPermissionScreen: Boolean = true) {
        thread {
            synchronized(this) {
                if (isSignedIn()) {
                    clearUserDetails()
                    com.healthtrack.prefs.SharedPref.setStringParams(
                        com.healthtrack.CoronaApplication.instance,
                        com.healthtrack.prefs.SharedPrefsConstants.UNIQUE_ID,
                        ""
                    )
                }
                if (com.healthtrack.background.BluetoothScanningService.serviceRunning) {
                    try {
                        val myService = Intent(
                            context,
                            com.healthtrack.background.BluetoothScanningService::class.java
                        )
